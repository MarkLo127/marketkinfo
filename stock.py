...
class Holding:
    ...
    def holder(symbol):
        ...
        if holders is not None:
            ...
            fig.update_layout(title=f"機構持股{symbol.ticker}比例前10名",annotations=[dict(text=f'{symbol.ticker}',x=0.5, y=0.5,font=dict(size=16),showarrow=False)])
            st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    def fund_holder(symbol):
        ...
        if funds is not None:
            ...
            fig.update_layout(title=f"ETF持股{symbol.ticker}比例前10名",annotations=[dict(text=f'{symbol.ticker}',x=0.5, y=0.5,font=dict(size=16),showarrow=False)])
            st.plotly_chart(fig, use_container_width=True)
        else:
